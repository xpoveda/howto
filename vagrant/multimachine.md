...
La maquina se podrá despues volver a levantar directamente con `virtualbox` aunque es mejor hacerlo con `vagrant up` para levantar todos los nodos o con `vagrant up NOMBRE_NODO` y acceder via `ssh`.
```
c:\HashiCorp\Vagrant\bin>vagrant up master
Bringing machine 'master' up with 'virtualbox' provider...
==> master: Checking if box 'bento/ubuntu-16.04' is up to date...
